...
def main():
    ...
    ubuntu_img = [img for img in cs.images.list() if "Ubuntu 12.04" in img.name][0]
    flavor_512 = [flavor for flavor in cs.flavors.list() if flavor.ram == 512][0]
    svr1 = threading.Thread(target=create_servers, args=[cs, ubuntu_img, flavor_512, 1])
    svr2 = threading.Thread(target=create_servers, args=[cs, ubuntu_img, flavor_512, 2])
    svr3 = threading.Thread(target=create_servers, args=[cs, ubuntu_img, flavor_512, 3])
    svr1.start()
    svr2.start()
    svr3.start()
    svr1.join()
